...
class SanrioInterface:
    ...
    def draw_timeline_screen(self):
        """Draw the timeline screen with Sanrio theme and enhanced hover effects"""
        # Fill background with bg_color first
        self.screen.fill(self.theme.colors["bg_color"])
        
        # Draw checkered background if available
        if "background" in self.theme.assets:
            bg = self.theme.assets["background"]
            bg_w, bg_h = bg.get_size()
            for y in range(0, self.window_size[1], bg_h):
        ...
        # Frame information - centered at top
        frame_text = f"Frame: {self.current_frame} / {self.total_frames}"
        frame_info = self.theme.create_text_surface(frame_text, "main")
        self.screen.blit(frame_info, (self.window_size[0]//2 - frame_info.get_width()//2, 30))
        
        # Show cursor mode indicator to help with debugging
        cursor_text = f"Mode: {self.cursor_mode.upper()}"
        cursor_surf = self.theme.create_text_surface(cursor_text, "title_secondary", (255, 50, 150))
        self.screen.blit(cursor_surf, (20, 30))
        ...
        if self.frame_image is not None:
            ...
            # Create styled frame with border decoration
            if "frame_border" in self.theme.assets:
                # Get the border asset
                frame_border = self.theme.assets["frame_border"]
                ...
                # Calculate the border dimensions
                border_width = fixed_w + (h_padding * 2)
                border_height = fixed_h + (v_padding * 2)
                ...
                # Fallback if frame border asset not available
                frame_rect = pygame.Rect(x-10, y-10, fixed_w+20, fixed_h+20)
                pygame.draw.rect(self.screen, self.theme.colors["border_color"], frame_rect, border_radius=10)
            
            # Draw any active process segments overlays
            for start, end in self.process_markers:
                if start <= self.current_frame <= end:
                    # Current frame is in a processing segment
                    segment_overlay = pygame.Surface((fixed_w, fixed_h), pygame.SRCALPHA)
                    segment_overlay.fill((255, 150, 200, 30))  # Pink highlight
                    self.screen.blit(segment_overlay, (x, y))
                    
                    # Show segment info
                    segment_text = self.theme.create_text_surface(
                        f"Processing segment: {start} - {end}", "small", (255, 50, 150)
                    )
                    self.screen.blit(segment_text, (x + 10, y + 10))
            
            # Draw active segment being created
            if self.selected_segment is not None:
                start, _ = self.selected_segment
                # Current frame is in a processing segment
                segment_overlay = pygame.Surface((fixed_w, fixed_h), pygame.SRCALPHA)
                segment_overlay.fill((255, 100, 150, 50))  # Pink highlight
                self.screen.blit(segment_overlay, (x, y))
                
                # Show segment info
                segment_text = self.theme.create_text_surface(
                    f"Creating segment: {start} - {self.current_frame}", "small", (255, 50, 150)
                )
                self.screen.blit(segment_text, (x + 10, y + 10))
            
            # Draw timeline slider with custom heart indicator
            slider_y = self.window_size[1] - 250 #MOVE UPWARDS
            slider_width = self.window_size[0] - 200
            slider_rect = pygame.Rect(100, slider_y, slider_width, 20)
            self.slider_rect = slider_rect  # Store for event handling
            
            # Draw slider background
            pygame.draw.rect(self.screen, (255, 200, 200), slider_rect, border_radius=10)
            pygame.draw.rect(self.screen, (200, 100, 150), slider_rect, 2, border_radius=10)
            
            # Draw process markers on timeline with flag icons
            if "flag_icon" in self.theme.assets:
                flag_icon = self.theme.assets["flag_icon"]
                flag_size = 24  # Size for the flag icon
                scaled_flag = pygame.transform.scale(flag_icon, (flag_size, flag_size))
            
            for start, end in self.process_markers:
                # Calculate positions on the timeline
                start_pos = 100 + int((start / self.total_frames) * slider_width)
                end_pos = 100 + int((end / self.total_frames) * slider_width)
                marker_width = max(end_pos - start_pos, 5)
                
                # Draw the segment marker
                marker_rect = pygame.Rect(start_pos, slider_y, marker_width, 20)
                pygame.draw.rect(self.screen, (255, 100, 180), marker_rect, border_radius=10)
                
                # Add flag icons at the start and end of each segment
                if "flag_icon" in self.theme.assets:
                    # Start flag
                    self.screen.blit(scaled_flag, (start_pos - flag_size//2, slider_y - flag_size))
                    
                    # End flag
                    self.screen.blit(scaled_flag, (end_pos - flag_size//2, slider_y - flag_size))
            
            # Draw current position indicator (heart or custom handle)
            pos_x = 100 + int((self.current_frame / self.total_frames) * slider_width)
            
            if "heart_slider" in self.theme.assets:
                heart = self.theme.assets["heart_slider"]
                heart_y = slider_y - heart.get_height() // 2 + 10
                self.screen.blit(heart, (pos_x - heart.get_width() // 2, heart_y))
            else:
                # Fallback if heart asset not available
                pygame.draw.circle(self.screen, (255, 50, 100), (pos_x, slider_y + 10), 10)
                
            # Flag icon button on left side
            flag_btn_x = 20
            flag_btn_y = self.window_size[1] - 125
            
            if "flag_icon" in self.theme.assets:
                flag_icon = self.theme.assets["flag_icon"]
                flag_btn_width, flag_btn_height = flag_icon.get_size()
                flag_btn_rect = pygame.Rect(flag_btn_x, flag_btn_y, flag_btn_width, flag_btn_height)
                self.screen.blit(flag_icon, flag_btn_rect)
                
                # Highlight if active or hovered
                is_flag_hovered = self.is_button_hovered("flag_icon", flag_btn_x, flag_btn_y)
                highlight = is_flag_hovered or self.cursor_mode == "flag"  # Highlight if in flag mode
                if highlight:
                    pygame.draw.rect(self.screen, (255, 100, 150), flag_btn_rect, 3, border_radius=5)
                self.flag_btn_rect = flag_btn_rect
                
            if "x_icon" in self.theme.assets:
                x_icon = self.theme.assets["x_icon"]
                # X icon for deleting segments - position next to back arrow button
                x_btn_x = flag_btn_x + 150
                x_btn_y = self.window_size[1] - 125
                x_btn_width, x_btn_height = x_icon.get_size()
                x_btn_rect = pygame.Rect(x_btn_x, x_btn_y, x_btn_width, x_btn_height)
                self.screen.blit(x_icon, x_btn_rect)
                
                # Highlight if active or hovered - using similar logic to flag_icon
                is_x_hovered = self.is_button_hovered("x_icon", x_btn_x, x_btn_y)
                highlight = is_x_hovered
                if highlight:
                    pygame.draw.rect(self.screen, (255, 100, 150), x_btn_rect, 3, border_radius=5)
                self.x_btn_rect = x_btn_rect
            
                        # Draw instruction panel using the themed asset if available

            # NEW - Back arrow button to undo last segment
            back_arrow_x = x_btn_x + 150
            back_arrow_y = self.window_size[1] - 125
            
            if "back_arrow" in self.theme.assets:
                back_arrow = self.theme.assets["back_arrow"]
                back_arrow_width, back_arrow_height = back_arrow.get_size()
                back_arrow_rect = pygame.Rect(back_arrow_x, back_arrow_y, back_arrow_width, back_arrow_height)
                self.screen.blit(back_arrow, back_arrow_rect)
                
                # Highlight if hovered
                is_back_arrow_hovered = self.is_button_hovered("back_arrow", back_arrow_x, back_arrow_y)
                if is_back_arrow_hovered:
                    pygame.draw.rect(self.screen, (255, 100, 150), back_arrow_rect, 3, border_radius=5)
                self.back_arrow_rect = back_arrow_rect
                
            if "instructions_panel" in self.theme.assets:
                instruction_panel = self.theme.assets["instructions_panel"]
                
                # Scale the panel to increase its size
                scaled_panel_width = int(instruction_panel.get_width() * 1.28)
                scaled_panel_height = int(instruction_panel.get_height() * 1.5)
                instruction_panel = pygame.transform.scale(instruction_panel, (scaled_panel_width, scaled_panel_height))
                
                panel_x = 20
                panel_y = 20
                self.screen.blit(instruction_panel, (panel_x, panel_y))
                
                # Get updated panel dimensions to better position text
                panel_width = instruction_panel.get_width()
                panel_height = instruction_panel.get_height()
                ...
                instruct_text = [
                    "Segment Selection",
                    "Click Flag to change to selection mode",
                    "Click timeline to start and stop processing",
                    "Click X to delete all segments",
                    "Click Back arrow to delete last segment"
                ]
                ...
                for i, text in enumerate(instruct_text):
                    ...
                    # Blit the text at the calculated position
                    self.screen.blit(text_surf, (text_x, text_y))
            # Play button - position to the right of X button
            play_btn_x = self.window_size[1] - 475
            play_btn_y = self.window_size[1] - 100
            play_btn_rect = pygame.Rect(play_btn_x, play_btn_y, 120, 40)
            
            # Draw themed play button
            if not self.is_playing:
                is_play_hovered = self.is_button_hovered("play_button", play_btn_x, play_btn_y)
                self.play_btn_rect = self.theme.draw_theme_button(
                    self.screen, "play_button", 
                    play_btn_x, play_btn_y,
                    hover=is_play_hovered
                )
            else:
                # Custom pause button
                pygame.draw.rect(self.screen, self.theme.colors["button_color"], play_btn_rect, border_radius=10)
                
                # Check if mouse is hovering over pause button
                is_pause_hovered = play_btn_rect.collidepoint(pygame.mouse.get_pos())
                if is_pause_hovered:
                    pygame.draw.rect(self.screen, self.theme.colors["highlight_color"], play_btn_rect, 3, border_radius=10)
                
                play_text = "⏸ Pause"
                play_text_surf = self.theme.create_text_surface(play_text, "small")
                self.screen.blit(play_text_surf, (
                    play_btn_rect.centerx - play_text_surf.get_width()//2, 
                    play_btn_rect.centery - play_text_surf.get_height()//2
                ))
                self.play_btn_rect = play_btn_rect
            
            # Crop button - position to the right of Play button
            crop_btn_x = self.window_size[1] - 300
            crop_btn_y = self.window_size[1] - 100
            
            # Check if mouse is hovering over crop button
            is_crop_hovered = self.is_button_hovered("crop_button", crop_btn_x, crop_btn_y)
            
            # Draw the crop button with hover effect
            self.crop_btn_rect = self.theme.draw_theme_button(
                self.screen, "crop_button", 
                crop_btn_x, crop_btn_y,
                hover=is_crop_hovered
            )
            
            # Next button - at bottom center
            next_btn_x = self.window_size[0]//2 + 600
            next_btn_y = self.window_size[1] - 150
            
            # Check if mouse is hovering over next button
            is_next_hovered = self.is_button_hovered("next_button", next_btn_x, next_btn_y)
            ...
            settings_btn_x = self.window_size[0]//2 + 550
            settings_btn_y = 10
            
            # Check if mouse is hovering over settings button
            is_settings_hovered = self.is_button_hovered("settings_button", settings_btn_x, settings_btn_y)
    ...
    def draw_export_screen(self):
        """Draw the export screen with Sanrio theme and enhanced hover effects"""
        # Draw background pattern
        if "background" in self.theme.assets:
            bg = self.theme.assets["background"]
            bg_w, bg_h = bg.get_size()
        ...
        # Draw export location header - MOVED UPWARD
        if "export_location_text" in self.theme.assets:
            header = self.theme.assets["export_location_text"]
            header_x = self.window_size[0]//2 - header.get_width()//2
            # Move the header upward, away from the input field
            header_y = self.window_size[1]//7  # Changed from //6 to //7 to move upward
            self.screen.blit(header, (header_x, header_y))
        else:
            # Fallback if asset not available
            header_text = self.theme.create_text_surface("Select export location", "title")
            self.screen.blit(header_text, (self.window_size[0]//2 - header_text.get_width()//2, self.window_size[1]//7))
        
        # Draw export path display with themed input field
        path_field = pygame.Rect(self.window_size[0]//2 - 300, self.window_size[1]//3, 600, 50)
        
        # Draw input field background
        pygame.draw.rect(self.screen, (255, 255, 255), path_field, border_radius=10)
        pygame.draw.rect(self.screen, self.theme.colors["border_color"], path_field, 3, border_radius=10)
        
        # Draw folder icon
        if "folder_icon" in self.theme.assets:
            folder = self.theme.assets["folder_icon"]
            # Position the folder icon in the left side of the input field
            folder_x = path_field.x + 15
            folder_y = path_field.y + (path_field.height - folder.get_height()) // 2
            self.screen.blit(folder, (folder_x, folder_y))
            text_offset = folder.get_width() + 30  # Increased offset for better spacing
        else:
            text_offset = 20
            folder_x = path_field.x + 10
        
        # Draw path text - CENTERED IN THE INPUT FIELD
        # Display just the folder name instead of full path for better fit
        if os.path.isdir(self.export_path):
            display_path = os.path.basename(self.export_path) or self.export_path
        else:
            display_path = self.export_path
            
        # Draw path text - vertically centered in the input field
        path_text = self.theme.create_text_surface(display_path, "title")
        # Calculate text position to be centered in the input field
        self.screen.blit(path_text, (folder_x + 200, folder_y + 65))
        
        # Draw segments summary
        segments_text = self.theme.create_text_surface(
            f"Processing {len(self.process_markers)} video segment(s)", "main_secondary"
        )
        self.screen.blit(segments_text, (self.window_size[0]//2 - segments_text.get_width()//2, self.window_size[1]//3 + 80))
        
        # Browse button
        browse_button_x = self.window_size[0]//2 - 100
        browse_button_y = self.window_size[1]//2 - 30
        
        # Check if mouse is hovering over browse button
        is_browse_hovered = self.is_button_hovered("browse_button", browse_button_x, browse_button_y)
        
        # Draw the browse button with hover effect
        self.browse_btn_rect = self.theme.draw_theme_button(
            self.screen, "browse_button", 
            browse_button_x, browse_button_y,
            hover=is_browse_hovered
        )
        
        # Finish button
        finish_button_x = self.window_size[0]//2 - 100
        finish_button_y = self.window_size[1]//2 + 150
        
        # Check if mouse is hovering over finish button
        is_finish_hovered = self.is_button_hovered("finish_button", finish_button_x, finish_button_y)
        
        # Draw the finish button with hover effect
        self.finish_btn_rect = self.theme.draw_theme_button(
            self.screen, "finish_button", 
            finish_button_x, finish_button_y,
            hover=is_finish_hovered
        )
        
        # Settings button
        settings_button_x = self.window_size[0]//2 + 350
        settings_button_y = 20
    ...
    def draw_settings_menu(self):
        """Draw the settings menu with Sanrio theme and enhanced hover effects"""
        ...
        # Draw striped background for menu panel if available
        if "striped_bg" in self.theme.assets:
            striped_bg = self.theme.assets["striped_bg"]
            panel_rect = pygame.Rect(self.window_size[0] - 350, 0, 350, self.window_size[1])
            
            # Scale and position the striped background
            scaled_bg = pygame.transform.scale(striped_bg, (panel_rect.width, panel_rect.height))
            self.screen.blit(scaled_bg, panel_rect)
        
        # Draw menu panel
        menu_surface = pygame.Surface((350, self.window_size[1]), pygame.SRCALPHA)
        menu_surface.fill((250, 220, 240, 230))  # Semi-transparent pink
        self.screen.blit(menu_surface, (self.window_size[0] - 350, 0))
        
        # Draw menu title
        if "graph_options" in self.theme.assets:
            title = self.theme.assets["graph_options"]
            title_x = self.window_size[0] - 350 + (350 - title.get_width()) // 2
            self.screen.blit(title, (title_x, 20))
        else:
            # Fallback if asset not available
            title = self.theme.create_text_surface("Graph Options", "title", self.theme.colors["highlight_color"])
            self.screen.blit(title, (self.window_size[0] - 350 + 20, 20))
        
        # Draw decorative divider
        if "divider" in self.theme.assets:
            divider = self.theme.assets["divider"]
            div_x = self.window_size[0] - 350 + (350 - divider.get_width()) // 2
            self.screen.blit(divider, (div_x, 60))
        else:
            # Fallback if asset not available
            pygame.draw.line(self.screen, self.theme.colors["highlight_color"], 
                        (self.window_size[0] - 330, 60), (self.window_size[0] - 20, 60), 2)
        
        # Draw checkboxes for processing options
        y_offset = 90
        checkboxes = self.get_checkboxes(y_offset)
        
        # Store checkboxes for event handling
        self.option_checkboxes = checkboxes
        
        # Draw close button with themed asset
        if "save_close" in self.theme.assets:
            save_close = self.theme.assets["save_close"]
            save_x = self.window_size[0] - 350 + (350 - save_close.get_width()) // 2
            save_y = self.window_size[1] - save_close.get_height() - 20
            
            # Create rectangle for the button
            save_rect = pygame.Rect(save_x, save_y, save_close.get_width(), save_close.get_height())
            
            # Check for hover with expanded hover area
            save_hover_rect = pygame.Rect(
                save_x - 20,
                save_y - 20,
                save_close.get_width() + 40,
                save_close.get_height() + 40
            )
            is_close_hovered = save_hover_rect.collidepoint(pygame.mouse.get_pos())
            
            # Apply hover effect if hovering
            if is_close_hovered:
                # Draw hover effect
                pygame.draw.rect(self.screen, (255, 200, 220), save_rect, border_radius=10)
            
            self.screen.blit(save_close, (save_x, save_y))
            self.close_btn_rect = save_hover_rect  # Store the expanded hover rect for event handling
        else:
            # Fallback if asset not available
            close_btn_rect = pygame.Rect(self.window_size[0] - 200, self.window_size[1] - 60, 150, 40)
            
            # Create expanded hover rectangle
            close_hover_rect = pygame.Rect(
                close_btn_rect.x - 20,
                close_btn_rect.y - 20,
                close_btn_rect.width + 40,
                close_btn_rect.height + 40
            )
            is_close_hovered = close_hover_rect.collidepoint(pygame.mouse.get_pos())
            
            # Draw button with hover effect
            pygame.draw.rect(self.screen, self.theme.colors["button_color"], close_btn_rect, border_radius=10)
            if is_close_hovered:
                pygame.draw.rect(self.screen, self.theme.colors["highlight_color"], close_btn_rect, 3, border_radius=10)
            
            close_text = self.theme.create_text_surface("Save & Close", "small")
            self.screen.blit(close_text, (close_btn_rect.centerx - close_text.get_width()//2, 
                                        close_btn_rect.centery - close_text.get_height()//2))
            self.close_btn_rect = close_hover_rect  # Store the expanded hover rect for event handling

    def get_checkboxes(self, start_y_offset=90):
        """Create and draw checkboxes for processing options"""
        checkboxes = {}
        ...
        for category, options in categories.items():
            # Draw category header with themed font
            category_text = self.theme.create_text_surface(category, "main", self.theme.colors["highlight_color"])
            self.screen.blit(category_text, (self.window_size[0] - 330, y_offset))
            y_offset += 35
            
            for option in options:
                # Verify the option exists in processing_options
                if option not in self.processing_options:
                    continue
                    
                # Format the label (convert snake_case to friendly name)
                label = " ".join(word.capitalize() for word in option.split("_")[1:]) if option.startswith("generate_") or option.startswith("export_") else " ".join(word.capitalize() for word in option.split("_"))
                
                # Create checkbox rect
                checkbox_rect = pygame.Rect(self.window_size[0] - 320, y_offset, 20, 20)
                ...
                is_hovered = checkbox_rect.collidepoint(pygame.mouse.get_pos())
                
                # Draw checkbox border
                border_color = self.theme.colors["highlight_color"] if is_hovered else self.theme.colors["border_color"]
                pygame.draw.rect(self.screen, border_color, checkbox_rect, 2, border_radius=3)
                
                # Draw checkbox fill if checked
                if self.processing_options.get(option, False):
                    inner_rect = pygame.Rect(checkbox_rect.x + 4, checkbox_rect.y + 4, 
                                        checkbox_rect.width - 8, checkbox_rect.height - 8)
                    pygame.draw.rect(self.screen, (255, 100, 180), inner_rect, border_radius=2)
                
                # Draw label text
                label_text = self.theme.create_text_surface(label, "small")
                self.screen.blit(label_text, (checkbox_rect.right + 10, 
                                            checkbox_rect.y + (checkbox_rect.height - label_text.get_height())//2))
